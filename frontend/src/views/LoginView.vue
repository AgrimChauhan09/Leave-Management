  ...
  <div class="mx-auto max-w-md space-y-4">
    ...
    <form class="space-y-3 rounded-lg border bg-white p-4" @submit.prevent="onSubmit">
      ...
      <div>
        ...
        <input v-model="form.password" type="password" class="mt-1 w-full rounded-md" required />
      </div>
    ...
    <p class="text-sm text-slate-700">
      No account?
      <router-link class="font-medium text-slate-900 underline" :to="{ name: 'signup' }">Sign up</router-link>
    </p>
  </div>
</template>
